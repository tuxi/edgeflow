...
func send(mailTo []string, subject, body string) error {
	...
	m.SetHeader("From", mailcfg.Sender)
	m.SetHeader("To", mailTo...)
	m.SetHeader("Subject", subject)
	m.SetBody("text/html", body)

	d := gomail.NewDialer(mailConn["host"], port, mailcfg.Sender, mailConn["pass"])
	if mailcfg.ProxyURL.Mode == "socks5" {
		gomail.NetDialTimeout = func(network string, address string, timeout time.Duration) (net.Conn, error) {
			proxyAdd := fmt.Sprintf("%s:%s", mailcfg.ProxyURL.Ip, mailcfg.ProxyURL.Port)
